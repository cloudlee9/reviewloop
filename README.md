...
没达标时多一行「还差什么」：`还差 交付物 差 1.5 分；2 个阻塞项要清零`。门槛是三个条件的与，而人盯着看的往往只有分数——有人跑满 10 轮，以为卡在生产就绪度上（那一直在涨），实际卡的是始终没归零的阻塞项。**只报差距，不预测"还要几轮"**：分数不是线性涨的，最后一段最难，一个不靠谱的预测比不给更糟。
...
除了达标（退 0）和跑满轮数，还有几个提前收场的出口 —— 都是为了不让你烧着配额等一个不会来的结果：

| 出口 | 什么时候 | 意思 |
|---|---|---|
| `converged` | 双分达标且阻塞项归零 | 停，退 0 |
| `stalled` | 连续几轮分数纹丝不动 | 熔断 |
| `deadlocked` | 某条 finding 连续 3 轮 `not_fixed`，而你**每轮都写了实质回应** | 双方顶住了，交给人 |
| `inconsistent` | reviewer 自相矛盾 | 判定不可信，退 3 |
| `pinned_scope` | 范围钉在历史提交上 | 改工作区进不了 diff，只出报告 |

`deadlocked` 值得单说。未修好的 finding 会被强制重列进 `findings`，而门禁要求阻塞项归零——**一条双方都不让步的 high finding，会让这个 loop 在数学上不可能通过**，只能白跑到轮数上限。真实案例里这浪费了七轮。

它**不是放行**：判定仍是未达标，findings 原样交给你。刻意没做成"作者声称需要产品决策就不算阻塞"——那等于开一条绕过门禁的通路，而在那个案例里 reviewer 到最后一轮仍坚持问题实存。它只决定"还要不要接着烧配额"。

判据要两边都占：reviewer 连续判 `not_fixed`，**并且**你每轮都有实质回应。你不吭声不算僵局，那叫没干活。
